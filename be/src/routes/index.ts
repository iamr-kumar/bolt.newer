...
interface TokenUsageRequest extends Request {
  body: {
    apiKey?: string;
    startDate?: string;
    endDate?: string;
...
/**
 * @route GET /api/test
 * @desc Test route to check if Anthropic API is working
 * @access Public
 */
router.get(
  "/test",
  async (req: TokenUsageRequest, res: Response, next: NextFunction) => {
    try {
      const response = await anthropic.messages.create({
